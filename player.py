...
def main(tracks_dir):
    ...
    for track_folder in track_folders:
        ...
        for f in files:
            ...
            output_subfolder = os.path.join(temp_folder, os.path.basename(track_folder))
    ...
    try:
        while True:
            if playing:  # Only play if the user has started playback
                try:
                    my_art = AsciiArt.from_image(os.path.join(track_folders[track_index], 'artwork.gif'))
                    my_art.to_terminal(columns=48, char='█', width_ratio=2)
                    print()
